...
def main(args):
    ...
    X_simulated = X_simulated[:, args.feature_start:args.feature_end]
    y_simulated = np.loadtxt(f'{args.path}T1T2_SimLabels_300.txt')  # Labels for simulated data

    X_experimental = np.loadtxt(f"{args.path}Experiments_13_99TallestPeaks.txt")  # Frequency list for experimental data
    X_experimental = X_experimental[:, args.feature_start:args.feature_end]
    y_experimental = np.loadtxt(f"{args.path}Experiments_13_Labels.txt")  # Labels for experimental data
    ...
    plt.tight_layout()
        ...
        # ==== BOXPLOT OF TRAINING AND TESTING ACCURACIES ====
    plt.figure(figsize=(8, 6))
    plt.boxplot([accuracies_train, accuracies_test], labels=['Train', 'Test'], patch_artist=True,
                boxprops=dict(facecolor='lightblue'), medianprops=dict(color='black'))
    plt.ylabel("Accuracy")
    plt.title("Boxplot of Training and Testing Accuracies")
    plt.grid(True)
    plt.tight_layout()
    plt.show()
    plt.show()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--feature_start', type=int, default=0, help="Start index of features to include (inclusive)")
    parser.add_argument('--feature_end', type=int, default=None, help="End index of features to include (exclusive)")
